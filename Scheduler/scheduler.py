...
class Scheduler(QMainWindow):
    def __init__(self, language, theme, filepath=None):
        ...
        self.table.loadData([])
        self.setGeometry(QRect(0, 0, 981, 764))
        ...
        extractPdf = QAction(variable(Var.EXTRACTPDF, self.language), menu)
        extractPdf.triggered.connect(self.make_pdf)
        menu.addAction(extractPdf)
    ...
    def save_file(self):
        ...
        # Trying to save a schedule that have not been generated by the tool
        if self.data == {}:
            PopupMessage(variable(Var.BAD_SCHEDULE, self.language))
            return
    ...
    def make_pdf(self):
        pdfPath = filePicker(True, ext="pdf")
        data = self.table.extractData()
        print(data)
        tabler = Tabler()
        for day in range(1, 8):
            _data = data[day - 1]
            for title, placement in _data.items():
                for index in range(24):
                    if (placement >> index) & 1:
                        tabler.fill_cell(day, index, title)
        tabler.save_pdf(pdfPath)
